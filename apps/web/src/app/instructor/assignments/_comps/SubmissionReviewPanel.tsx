  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      <div className="lg:col-span-2 space-y-4">
        <div className="glass-card p-5 border border-border/80 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={onBack} className="text-xs text-muted hover:text-foreground font-medium">
              ← Back to assignments
            </button>
          </div>
          ...
          <div className="flex flex-wrap gap-4 text-xs text-muted-foreground mt-2">
            <span className="flex items-center gap-1">📅 Due: {new Date(selectedAssignment.dueDate).toLocaleString()}</span>
            <span className="flex items-center gap-1">💯 Max Points: {selectedAssignment.maxPoints}</span>
          </div>
        </div>
        ...
        <div className="glass-card p-5 border border-border/80">
          ...
          {loadingSubmissions ? (
            <div className="text-center py-8 text-sm text-muted animate-pulse">Fetching student submissions...</div>
          ) : submissions.length === 0 ? (
            <div className="text-center py-8 text-sm text-muted-foreground">No submissions recorded yet for this assignment.</div>
          ) : (
                ...
                <tbody className="divide-y divide-border/40">
                  {submissions.map((sub) => (
                    ...
                    >
                      ...
                      <td className="py-3 pr-2 text-muted">{new Date(sub.submittedAt).toLocaleDateString()}</td>
                      ...
                      <td className="py-3 pr-2">
                        <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${sub.status === "GRADED" ? "bg-emerald-500/10 text-emerald-400" : "bg-amber-500/10 text-amber-400"}`}>
      ...
      <div className="space-y-4">
        <div className="glass-card p-5 border border-border/80">
          <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground mb-4">Evaluation Assistant</h3>

          {submissionDetailLoading ? (
            <div className="text-center py-8 text-sm text-muted animate-pulse">Loading submission details...</div>
          ) : !selectedSubmission ? (
            ...
            <div className="space-y-5">
              <div className="p-3.5 rounded-xl border border-border/80 bg-background/50">
                ...
                <div className="grid grid-cols-2 gap-4 mt-3 pt-3 border-t border-border/40">
                  ...
                  <div>
                    ...
                    <p className={`text-xs font-bold ${selectedSubmission.status === "GRADED" ? "text-emerald-400" : "text-amber-400"}`}>
                      ...
                      <div key={q.id} className="p-3 rounded-lg border border-border/50 bg-background/30 text-xs">
                        ...
                        <div className="mt-2 space-y-1.5 pl-1.5">
                          {q.options.map((o) => {
                            ...
                            return (
                              <div key={o.id} className={`flex items-start gap-1.5 p-1 rounded ${o.isCorrect ? "bg-emerald-500/10 text-emerald-400 font-semibold" : isSelected ? "bg-danger/10 text-danger" : "text-muted-foreground"}`}>
                                <span className="mt-0.5 shrink-0">{o.isCorrect ? "✅" : isSelected ? "❌" : "○"}</span>
              ...
              {selectedSubmission.assignment.type === "ASSIGNMENT" && (
                <div className="space-y-3">
                  ...
                  {selectedSubmission.assignment.questionPdfUrl && (
                    <a href={selectedSubmission.assignment.questionPdfUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 p-3 rounded-lg border border-border/50 bg-background/30 text-xs hover:border-primary/40 transition-colors">
                      <span>📄</span>
                  ...
                  {selectedSubmission.answerFileUrl ? (
                    <a href={selectedSubmission.answerFileUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 p-3 rounded-lg border border-border/50 bg-background/30 text-xs hover:border-primary/40 transition-colors">
                      <span>📎</span>
              ...
              <form onSubmit={handleGradeSubmission} className="space-y-3 pt-3 border-t border-border/60">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-muted uppercase tracking-wider">Final Score / Grade</label>
                  <input type="text" required value={gradeInput} onChange={(e) => setGradeInput(e.target.value)} className="field py-1.5 text-xs" placeholder="e.g. 8/10 or A+" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-muted uppercase tracking-wider">Grader Feedback Notes</label>
                  <textarea rows={3} value={feedbackInput} onChange={(e) => setFeedbackInput(e.target.value)} className="field text-xs" placeholder="Provide advice, study tips, or congrats..." />
                </div>
                {gradingSuccess && (
                ...
                <button type="submit" disabled={submitting} className="btn-primary w-full text-xs py-2 justify-center">
