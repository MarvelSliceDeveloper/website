      ...
      <div className="flex-1 overflow-y-auto">
        {data.modules.length === 0 ? (
          ...
          data.modules.map((mod, idx) => {
            ...
            const inProgress =
              mod.completionPercent > 0 && mod.completionPercent < 100;
            const modRecordings = data.recordings.filter(
              (r) => r.moduleId === mod.id,
            );
                ...
                >
                  ...
                  >
                    {isComplete ? (
                      ...
                      <IconPlayerPlay size={9} className="ml-0.5" />
                    ) : (
                      idx + 1
                    )}
                  </div>
                  <span
                    className={`text-[12px] font-medium flex-1 truncate ${isSelected ? "text-primary" : "text-foreground"}`}
                  >
                    {mod.title}
                  </span>
                  ...
                  >
                    <IconBookmark
                      ...
                      fill={
                        bookmarks.includes(mod.id) ? "currentColor" : "none"
                      }
                    />
                  </button>
                  {isExpanded ? (
                    <IconChevronUp size={18} className="text-muted shrink-0" />
                  ) : (
                    <IconChevronDown
                      size={18}
                  ...
                  <div className="pl-11 pr-3 pb-1.5 space-y-0.5">
                    ...
                    {mod.lessons.map((lesson) => {
                      const isLessonSelected =
                        selectedLessonId === lesson.id && !selectedRecordingId;
                      return (
                        <div
                          key={lesson.id}
                          onClick={() => {
                        ...
                        >
                          ...
                          <span
                            className={`text-xs flex-1 truncate ${isLessonSelected ? "text-primary font-medium" : "text-muted-foreground"}`}
                          >
                            {lesson.title}
                          </span>
                    ...
                    {modRecordings.map((rec) => (
                      ...
                      >
                        ...
                        />
                        <span
                          className={`text-xs flex-1 truncate ${selectedRecordingId === rec.id ? "text-primary font-medium" : "text-muted-foreground"}`}
                        >
                    ...
                    {mod.lessons.length === 0 && modRecordings.length === 0 && (
                      <p className="py-1.5 text-[11px] text-muted italic">
