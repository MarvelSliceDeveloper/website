    ...
    <div className="sp-view-enter space-y-6">
      ...
      {grouped.every((g) => g.items.length === 0) ? (
        <div className="glass-card flex flex-col items-center gap-3 py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground">No live sessions scheduled today. Check back later.</p>
        </div>
      ) : (
        grouped.map((group) => (
          <div key={group.label} className="space-y-3">
            {group.label && (
              <p className="sp-eyebrow">{group.label}</p>
            )}
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="mb-2 flex flex-wrap items-center gap-2">
            {isLive && (
              <span className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-bold uppercase tracking-widest text-danger">Live Now</span>
              </span>
            )}
          ...
          <p className="mt-0.5 text-xs text-muted">
            ...
            {!isLive && ` · ${scheduledStr}`}
            {isLive && ` · Started ${Math.max(0, Math.floor((Date.now() - new Date(session.scheduledAt).getTime()) / 60000))} min ago`}
          </p>
        </div>
