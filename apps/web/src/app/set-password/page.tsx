  ...
  if (checking || loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#EEF2F7]">
        <div className="h-10 w-10 animate-spin rounded-full border-4 border-[#FF5E14] border-t-transparent" />
      </div>
    );
  }
        ...
        <section className="hidden lg:flex relative bg-gradient-to-br from-[#0055FE] via-[#0047BA] to-[#0038A8] p-8 xl:p-10 flex-col justify-between overflow-hidden text-white lg:min-h-[500px]">
          ...
          <div className="relative z-10 pt-3">
            ...
            <p className="mt-2.5 text-xs xl:text-sm text-white/85 leading-relaxed max-w-sm font-medium">
              Your account was created with temporary credentials. Set a permanent password to secure your learning journey.
            </p>
          </div>
          ...
          <div className="relative z-10 my-4 space-y-3">
            <div className="bg-white/95 backdrop-blur-md rounded-2xl p-4 shadow-lg border border-white/80 flex items-center gap-4 text-slate-800 transition-transform hover:scale-[1.01]">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Encrypted credentials store for absolute safety.
                </p>
              </div>
            </div>
            ...
            <div className="bg-white/95 backdrop-blur-md rounded-2xl p-4 shadow-lg border border-white/80 flex items-center gap-4 text-slate-800 transition-transform hover:scale-[1.01]">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Unlock full portal features right after saving.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="w-full max-w-[440px] mx-auto">
            ...
            <form onSubmit={handleSubmit} className="mt-5 space-y-3.5">
              ...
              <div>
                ...
                <div className="relative group flex items-center rounded-2xl border border-slate-200/90 bg-slate-50/80 p-1 transition-all duration-200 hover:border-slate-300 hover:bg-slate-100/60 focus-within:bg-white focus-within:border-[#FF5E14] focus-within:ring-2 focus-within:ring-[#FF5E14]/20 shadow-xs">
                  <div className="w-9 h-9 rounded-xl bg-orange-500/10 text-[#FF5E14] flex items-center justify-center shrink-0 transition-colors group-focus-within:bg-[#FF5E14] group-focus-within:text-[#FF5E14] shadow-xs">
                    <IconLock className="w-4 h-4" />
                  </div>
                  <input
                    type="password"
              ...
              <div className="space-y-1.5 rounded-xl border border-slate-200/80 bg-slate-50/60 p-3">
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`h-2 w-2 rounded-full transition-colors ${
                      passwordChecks.min ? "bg-emerald-500" : "bg-slate-300"
                    }`}
                  />
                  <span
                    className={`text-xs font-medium ${
                      passwordChecks.min ? "text-emerald-700 font-semibold" : "text-slate-500"
                    }`}
                  >
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`h-2 w-2 rounded-full transition-colors ${
                      passwordChecks.upper ? "bg-emerald-500" : "bg-slate-300"
                    }`}
                  />
                  <span
                    className={`text-xs font-medium ${
                      passwordChecks.upper ? "text-emerald-700 font-semibold" : "text-slate-500"
                    }`}
                  >
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`h-2 w-2 rounded-full transition-colors ${
                      passwordChecks.lower ? "bg-emerald-500" : "bg-slate-300"
                    }`}
                  />
                  <span
                    className={`text-xs font-medium ${
                      passwordChecks.lower ? "text-emerald-700 font-semibold" : "text-slate-500"
                    }`}
                  >
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`h-2 w-2 rounded-full transition-colors ${
                      passwordChecks.digit ? "bg-emerald-500" : "bg-slate-300"
                    }`}
                  />
                  <span
                    className={`text-xs font-medium ${
                      passwordChecks.digit ? "text-emerald-700 font-semibold" : "text-slate-500"
                    }`}
                  >
                    One number
                  </span>
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`h-2 w-2 rounded-full transition-colors ${
                      passwordChecks.match ? "bg-emerald-500" : "bg-slate-300"
                    }`}
                  />
                  <span
                    className={`text-xs font-medium ${
                      passwordChecks.match ? "text-emerald-700 font-semibold" : "text-slate-500"
                    }`}
                  >
                    Passwords match
                  </span>
              ...
              <label className="flex items-start gap-2.5 cursor-pointer select-none text-xs text-slate-600 font-medium pt-1">
                <input
                  type="checkbox"
                  checked={agreed}
                  onChange={(e) => setAgreed(e.target.checked)}
                  className="w-3.5 h-3.5 mt-0.5 rounded border-slate-300 text-[#0052CC] accent-[#0052CC] focus:ring-0 cursor-pointer shrink-0"
                />
