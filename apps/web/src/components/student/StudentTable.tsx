                ...
                <table className="w-full min-w-195">
                    ...
                    <tbody className="divide-y divide-border/60">
                        {rows.length === 0 ? (
                            ...
                            rows.map((row, index) => (
                                <tr key={rowKey(row, index)} className="transition-colors hover:bg-card-hover/50">
                                    {columns.map((column) => (
                                        <td key={column.key} className={`px-5 py-3 text-sm text-foreground ${column.className ?? ""}`}>
                                            {column.render(row)}
                                        </td>
                                    ))}
                                </tr>
