...
export default function OverdueAssignmentsPanel({
    ...
    const filtered = useMemo(() => items.filter((item) => item.status === filter), [items, filter]);
    ...
    const columns: StudentTableColumn<OverdueAssignmentItem>[] = [
        {
            key: "course",
            header: "Course Name",
            render: (row) => <span className="font-medium text-foreground">{row.courseName}</span>,
        },
        {
            key: "unit",
            header: "Unit Name",
    ...
    return (
        <section className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <h2 className="text-2xl font-bold text-foreground">{filtered.length} {title}</h2>
                <div className="flex gap-2">
                    <button
                        ...
                        className={`rounded-lg border px-4 py-2 text-sm font-medium transition-colors ${filter === "PENDING"
                    ...
                    <button
                        ...
                        className={`rounded-lg border px-4 py-2 text-sm font-medium transition-colors ${filter === "SUBMITTED"
                                ? "border-primary/30 bg-primary/20 text-primary"
                                : "border-border bg-card text-muted-foreground hover:bg-card-hover hover:text-foreground"
                            }`}
                    >
