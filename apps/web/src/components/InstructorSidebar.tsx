...
function NavGroup({
  ...
  return (
    <div className="space-y-1.5">
      <p
        className={`px-4 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 border-b border-border/40 ${collapsed ? "hidden" : "block"
          }`}
      >
        {label}
      </p>
              ...
              {hasChildren ? (
                collapsed ? (
                  <button
                    ...
                    className={`w-full flex items-center justify-center p-3 text-[13.5px] font-semibold transition-colors cursor-pointer border-l-3 ${navItemColors}`}
                  >
...
export default function InstructorSidebar({
  ...
  return (
    <aside
      className={`fixed left-0 top-14 z-40 hidden h-[calc(100vh-56px)] flex-col border-r border-t border-border bg-card transition-[width] duration-200 lg:flex ${collapsed ? "w-16" : "w-64"
        }`}
    >
      {/* Navigation Links */}
