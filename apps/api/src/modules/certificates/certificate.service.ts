...
async function buildCourseCompletionMap(userId: string) {
    ...
    if (courseIds.length === 0) {
        return { certificates: [] as CertificateSummary[], claimable: [] as ClaimableCertificate[] };
    }
    ...
    const batchToCourse = new Map(batches.map((batch) => [batch.id, batch.courseId]));
    ...
    const recordingsByCourse = new Map<string, { total: number; completed: number }>();
    ...
    const issued = certificates
        .map((certificate) => {
            ...
            const stats = recordingsByCourse.get(certificate.courseId) || { total: 0, completed: 0 };
