...
const WEBHOOK_CLIENT_STATE = process.env.MS_WEBHOOK_CLIENT_STATE || 'secretClientValue';
      ...
      for (const notification of notifications) {
        try {
          ...
          if (!resourceParts || resourceParts.length < 4) {
            console.warn('[EventsWebhook] Unrecognized resource format:', resource);
            continue;
          }
          ...
          if (!user) {
            console.warn(`[EventsWebhook] No user found for MS userId: ${msUserId}`);
            continue;
          }
          ...
          console.error('[EventsWebhook] Error processing notification:', notifError.message);
        }
      }
    } catch (error: any) {
...
async function handleEventCreatedOrUpdated(userId: string, msEventId: string) {
  ...
  const scheduledEndAt = event.end?.dateTime ? new Date(event.end.dateTime + 'Z') : undefined;
  ...
  if (existingSession) {
    ...
    const updateData: any = {
      scheduledAt,
      scheduledEndAt: event.end?.dateTime ? new Date(event.end.dateTime + 'Z') : undefined,
      joinUrl,
    };
    ...
    console.log(`[EventsWebhook] Updated existing session: ${existingSession.id}`);
    return;
  }
  ...
  if (!instructorBatch) {
    ...
    console.log(`[EventsWebhook] Calendar event upserted (no course link): ${msEventId}`);
    return;
  }
  ...
  if (!firstModule) {
    console.warn(`[EventsWebhook] Course ${instructorBatch.courseId} has no modules, skipping LiveSession creation`);
    return;
  }
  ...
  if (result.created) {
    console.log(`[EventsWebhook] Created LiveSession from Teams: ${result.session.id}`);
  } else {
...
async function handleEventDeleted(msEventId: string) {
  ...
  if (calendarEvent?.sessionId) {
    ...
    console.log(`[EventsWebhook] Marked session ${calendarEvent.sessionId} as ended (event deleted)`);
  }
