      ...
      for (const notification of notifications) {
        try {
          ...
          if (!resourceParts || resourceParts.length < 3) {
            console.warn('[Webhook] Unrecognized resource format:', notification.resource);
            continue;
          }
          ...
          await syncCalendarForUser(user.id, now.toISOString(), thirtyDaysLater.toISOString());

          console.log(`[Webhook] Calendar re-synced for user ${user.id}`);
        } catch (notifError: any) {
          // Log but don't fail — process remaining notifications
          console.error('[Webhook] Error processing notification:', notifError.message);
        }
      }
    } catch (error: any) {
      console.error('[Webhook] Error handling calendar webhook:', error.message);
