...
export const calendarController = {
  ...
  async syncCalendar(req: AuthRequest, res: Response) {
    try {
      ...
      const result = await syncCalendarForUser(req.user.userId, startDate, endDate);
      ...
      if (error.message.includes('Microsoft account not linked')) {
        return res.status(400).json({ error: 'Microsoft account not linked. Please connect your Microsoft account first.' });
      }
