          ...
          <form onSubmit={handleDemoSubmit} className="space-y-4">
            <div>
              <input
                ...
                value={demoForm.name}
                onChange={(e) =>
                  setDemoForm((p) => ({ ...p, name: e.target.value }))
                }
            ...
            <div>
              <input
                ...
                onChange={(e) =>
                  setDemoForm((p) => ({ ...p, phone: e.target.value }))
                }
            ...
            {demoMsg && <p className="!text-red-500 text-xs">{demoMsg.text}</p>}
...
export default function Career() {
  ...
  useEffect(() => {
    if (selectedJob) {
      setForm((prev) => ({
        ...
        category:
          selectedJob._type === "intern"
            ? "Internship"
            : selectedJob.type || prev.category,
      }));
    }
  }, [selectedJob]);
  ...
  const isLoading =
    pageLoading || catsLoading || jobsLoading || internshipsLoading;
  ...
  function renderForm() {
    ...
    const fieldDefs = {
      full_name: {
        type: "text",
        ...getFieldConfig(formCfg, "full_name", {
          label: "Full Name",
          enabled: true,
          required: true,
      ...
      position: {
        type: "text",
        ...getFieldConfig(formCfg, "position", {
          label: "Position",
          enabled: true,
          required: true,
        }),
      ...
      category: {
        type: "select",
        ...getFieldConfig(formCfg, "category", {
          label: "Category",
          enabled: true,
          required: true,
      ...
      description: {
        type: "textarea",
        ...getFieldConfig(formCfg, "description", {
          label: "Description",
          enabled: true,
          required: true,
      ...
      file_upload: {
        type: "file",
        ...getFieldConfig(formCfg, "file_upload", {
          label: "Upload Resume",
          enabled: true,
          required: true,
        }),
        ...
        <form ref={formRef} onSubmit={handleSubmit} noValidate>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-6 sm:p-8">
            {fieldDefs.full_name.enabled !== false && (
              <Field
                label={fieldDefs.full_name.label}
                required={fieldDefs.full_name.required !== false}
            ...
            {fieldDefs.email.enabled !== false && (
              <Field
                label={fieldDefs.email.label}
                required={fieldDefs.email.required !== false}
                error={errors.email}
              >
                <input
                  name="email"
            ...
            {fieldDefs.phone.enabled !== false && (
              <Field
                label={fieldDefs.phone.label}
                required={fieldDefs.phone.required !== false}
                error={errors.phone}
              >
                <input
                  name="phone"
            ...
            {fieldDefs.position.enabled !== false && (
              <Field
                label={fieldDefs.position.label}
                required={fieldDefs.position.required !== false}
            ...
            {fieldDefs.category.enabled !== false && (
              <Field
                label={fieldDefs.category.label}
                required={fieldDefs.category.required !== false}
                    ...
                    <>
                      ...
                      >
                        <option value="" disabled>
                          {fieldDefs.category.placeholder}
                        </option>
                        {(fieldDefs.category.options || []).map((c) => (
            ...
            {fieldDefs.description.enabled !== false && (
              <div className="sm:col-span-2">
                <Field
                  ...
                  required={fieldDefs.description.required !== false}
            ...
            {fieldDefs.file_upload.enabled !== false && (
              <div className="sm:col-span-2">
                <Field
                  label={fieldDefs.file_upload.label}
                  required={fieldDefs.file_upload.required !== false}
                  error={errors.file}
                >
                  <label
  ...
  return (
    <div>
          ...
          {roleCategories?.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8 max-w-6xl mx-auto text-left">
              {roleCategories.map((cat, idx) => (
                ...
                >
                  <div
                    className={`p-3 rounded-xl flex items-center justify-center shrink-0 ${idx % 2 === 0 ? "bg-brand-green/10 text-brand-green" : "bg-brand-orange/10 text-brand-orange"}`}
                  >
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-400">
                No role categories available right now.
              </p>
            </div>
          )}
        </div>
      ...
      <div
        ref={jobsRef}
        className="bg-gradient-to-b from-orange-50/40 via-slate-50 to-slate-50"
      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {(() => {
                      ...
                      for (let i = 0; i < maxLen; i++) {
                        if (internships?.[i])
                          allItems.push({ ...internships[i], _type: "intern" });
                        if (jobs?.[i])
                          allItems.push({ ...jobs[i], _type: "job" });
                      }
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <span className="text-xs font-semibold text-brand-orange bg-orange-50 px-2.5 py-1 rounded-full shrink-0">
                                  Internship
                                </span>
                              </div>
                              {(item.duration ||
                                ...
                                <div className="flex items-center gap-4 flex-wrap border-y border-slate-100 py-2.5 px-3 my-3 rounded-lg bg-orange-50/40 text-sm text-slate-600">
                                  ...
                                  {item.stipend && (
                                    <span className="flex items-center gap-1.5">
                                      <FiDollarSign className="w-3.5 h-3.5 text-brand-orange" />
                                      {item.stipend}
                                    </span>
                                  )}
                                  {item.experience && (
                                    <span className="flex items-center gap-1.5">
                                      <FiBriefcase className="w-3.5 h-3.5 text-brand-orange" />
                                      {item.experience}
                                    </span>
                                  )}
                                </div>
