    ...
    <section className="py-16 bg-gray-50/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Reveal as="p" className="text-gray-500 mb-6" delay={0.1}>
          A glimpse into the program experience
        </Reveal>
              ...
              <button
                onClick={() => {
                  ...
                  if (img.type === "video" || img.video_url)
                    trackVideoPlay(img.title || "service_gallery");
                }}
...
export default function ServiceDetail() {
  ...
  const ModeIcon =
    MODE_ICONS[service.training_mode?.toLowerCase().replace(/\s+/g, "-")] ||
    FiLayers;
  ...
  return (
    <div>
            ...
            <Reveal variant="up" delay={0.25}>
              <div className="flex flex-wrap gap-3">
                <Button
                  ...
                  size="lg"
                  to="/contact"
                  onClick={() => trackCtaClick("Enroll Now", "service_hero")}
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 lg:sticky lg:top-24">
                ...
                <div className="divide-y divide-gray-100">
                  ...
                  {service.difficulty_level && (
                    <InfoBadge
                      label="Difficulty"
                      value={service.difficulty_level}
                      icon={FiBarChart2}
                    />
                  )}
                  <YesNoBadge
                    label="Certificate"
                    value={service.certificate_available}
                    icon={FiAward}
                  />
                  <YesNoBadge
                    label="Placement Support"
                    value={service.placement_support}
                    icon={FiBriefcase}
                  />
                  <YesNoBadge
                    label="Internship"
                    value={service.internship_available}
                    icon={FiStar}
                  />
                </div>
              </div>
      ...
      {curriculum.length > 0 && (
        <section className="py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <Reveal as="p" className="text-gray-500 mb-8" delay={0.1}>
              Structured modules designed for practical learning
            </Reveal>
            <Reveal>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <Stagger className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl">
              {requirements.map((req, i) => {
                const label =
                  typeof req === "string" ? req : req.title || req.label;
                return (
                  <StaggerItem key={i}>
                  ...
                  <Reveal key={i} delay={0.05 * i}>
                    <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm h-full">
                      <div className="w-10 h-10 rounded-xl bg-brand-orange/10 flex items-center justify-center mb-4">
                        {(() => {
                          const IconComp =
                            HIGHLIGHT_ICONS[detail.icon] || FiLayers;
                          return (
      ...
      {/* ============ 12. FAQS ============ */}
      {service.service_faqs?.filter((f) => f.is_active !== false).length >
        0 && (
        <section className="py-16 bg-gray-50/50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <Reveal
            ...
            <Reveal>
              <Accordion
                items={service.service_faqs.filter(
                  (f) => f.is_active !== false,
                )}
              />
            </Reveal>
