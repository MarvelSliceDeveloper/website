  ...
  useEffect(() => {
    async function fetchUnread(type) {
      ...
      if (key === "chat") {
        const { data } = await supabase
          .from(table)
          .select(
            "id, user_name, last_message, last_message_sender, last_message_at, status",
          )
          .eq("notified", true)
    ...
    async function fetchAll() {
      const results = await Promise.all(
        submissionTypes.map((t) => fetchUnread(t)),
      );
      const map = {};
      submissionTypes.forEach((t, i) => {
        map[t.key] = results[i];
      });
      setUnreadByType(map);
    }
      ...
      return supabase
        ...
        .on(
          ...
          async (payload) => {
            ...
            toast({
              type: "success",
              message: `New ${t.label.toLowerCase()} submission`,
              duration: 4000,
            });
          },
        )
        ...
        <header className="sticky top-0 z-20 backdrop-blur-md bg-white/80 border-b border-admin-200/80 px-4 lg:px-6 flex items-center justify-between h-14 shrink-0 gap-4 transition-all">
          <div className="flex items-center gap-3 min-w-0">
            <button
              ...
              className="lg:hidden p-2 -ml-2 text-neutral-600 hover:bg-admin-100 rounded-lg transition-all duration-200"
            >
            ...
            <Link
              to="/admin"
              className="flex items-center gap-2 text-sm font-bold text-neutral-900 lg:hidden"
            >
              <img
                src={logoUrl}
          ...
          <div className="flex items-center gap-1">
            ...
            >
              ...
              <kbd className="text-[10px] text-neutral-400 bg-white border border-admin-200 rounded px-1.5 py-0.5 font-mono shadow-sm">
                ⌘K
              </kbd>
            </button>
            <button
              className="lg:hidden p-2 text-neutral-400 hover:text-neutral-600 hover:bg-admin-100 rounded-lg transition-all duration-200"
              onClick={() => setSearchOpen(true)}
            >
            ...
            <div className="relative" ref={notifRef}>
              <button
                ...
                className={`relative p-2 rounded-lg transition-all duration-200 ${notifOpen ? "bg-admin-100 text-admin-600" : "text-neutral-400 hover:text-neutral-600 hover:bg-admin-100"}`}
              >
                ...
                <div className="absolute right-0 top-full mt-2 w-[calc(100vw-2rem)] max-w-80 sm:w-80 bg-white rounded-xl shadow-2xl border border-admin-200 z-50 max-h-96 flex flex-col">
                  ...
                  <div className="overflow-y-auto admin-scrollbar flex-1">
                    {totalUnread === 0 ? (
                      ...
                      submissionTypes
                        .filter((t) => (unreadByType[t.key] || []).length > 0)
                        .map((type) => {
                          const items = unreadByType[type.key] || [];
                          const Icon = type.icon;
                          return (
                            <div key={type.key}>
                              <div className="px-4 py-2 text-[11px] font-semibold text-neutral-500 uppercase tracking-wider bg-gray-50/50">
                                {type.label}
                              </div>
                              {items.slice(0, 3).map((item, i) => (
                                <div
                                  key={item.id || i}
                                  className="group relative flex items-start gap-3 px-4 py-2.5 hover:bg-admin-50 transition-colors border-b border-admin-100 last:border-0"
                                >
                                  <Link
                                    to={type.link}
                                    onClick={() => {
                                      dismissNotification(type, item);
                                      setNotifOpen(false);
                                    }}
                                    className="flex items-start gap-3 flex-1 min-w-0"
                                  >
                                    <div
                                      className={`w-8 h-8 rounded-full ${type.bg} ${type.color} flex items-center justify-center shrink-0 mt-0.5`}
                                    >
            ...
            <Link
              to="/"
              target="_blank"
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm text-neutral-500 hover:text-neutral-700 hover:bg-admin-100 transition-all duration-200"
            >
            ...
            <div className="relative" ref={menuRef}>
              <button
                ...
                className={`flex items-center gap-2 ml-1 pl-3 pr-2 py-1.5 text-sm rounded-lg transition-all duration-200 ${menuOpen ? "bg-admin-100 text-admin-900" : "text-neutral-600 hover:text-neutral-900 hover:bg-admin-100"}`}
              >
                ...
                <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-xl shadow-2xl border border-admin-200 py-1 z-50">
                  <div className="px-3 py-2 text-xs text-neutral-500 border-b border-admin-100">
                    ...
                    <div className="capitalize mt-0.5">
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-white text-neutral-700">
                        {user?.role}
                      </span>
                    </div>
                  </div>
                  <Link
                    ...
                    className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:bg-admin-50 transition-all duration-200"
                  >
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-sm text-destructive-500 hover:bg-destructive-50 transition-all duration-200"
                  >
