...
import { FiLoader, FiBarChart2, FiPieChart, FiTrendingUp, FiActivity } from 'react-icons/fi';
...
export default function CourseReports() {
  ...
  const [dataByRange, setDataByRange] = useState({ today: {}, week: {}, month: {}, total: {} });
  ...
  useEffect(() => {
    async function load() {
      ...
      const startOfWeek = new Date(now.getFullYear(), now.getMonth(), now.getDate() - (dayOfWeek === 0 ? 6 : dayOfWeek - 1)).toISOString();
      ...
      for (const table of tableNames) {
        ...
        const { data } = await supabase.from(table).select('created_at').order('created_at', { ascending: true });
    ...
    if (chartType === 'pie') {
      return (
        <ResponsiveContainer width="100%" height={400}>
          <PieChart>
            <Pie data={data} cx="50%" cy="50%" outerRadius={130} dataKey="value" label={({ name, value }) => `${name}: ${value}`}>
      ...
      return (
        <ResponsiveContainer width="100%" height={400}>
          <LineChart data={data} margin={{ bottom: tickAngle ? 60 : 5 }}>
            ...
            <XAxis dataKey="name" tick={{ fontSize: 11, angle: tickAngle, textAnchor: tickAngle ? 'end' : 'middle' }} height={tickAngle ? 60 : 30} interval={0} />
  ...
  const availableChartOptions = useMemo(() => {
    return chartOptions.filter(o => {
      if (chartData.length > 8 && (o.key === 'pie' || o.key === 'radar')) return false;
      return true;
    });
  }, [chartData.length]);
  ...
  const rangeLabel = rangeOptions.find(r => r.key === selectedRange)?.label || '';

  if (loading) return (
    <PageShell backTo="/admin" title="Reports" subtitle="Submission statistics and analytics">
      <div className="flex items-center justify-center py-20"><FiLoader className="w-8 h-8 animate-spin text-admin-400" /></div>
    </PageShell>
  );
    ...
    <PageShell backTo="/admin" title="Reports" subtitle="Submission statistics and analytics">
      <div className="space-y-8">
        ...
        <div className="flex items-center gap-3">
          <label className="text-xs font-semibold text-neutral-500 uppercase tracking-wider">Time Range</label>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <p className="text-xs font-medium text-neutral-500 uppercase tracking-wider">{rangeLabel} Total</p>
            <p className="text-2xl font-bold mt-1 text-blue-600">{rangeData.total}</p>
          </div>
          {rangeData.byType.map(t => (
            <div key={t.name} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
              <p className="text-xs font-medium text-neutral-500 uppercase tracking-wider">{t.name}</p>
              <p className="text-2xl font-bold mt-1 text-neutral-700">{t.value}</p>
            </div>
          ))}
        </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex flex-wrap items-center justify-between gap-4">
            <h2 className="text-sm font-semibold text-black">{rangeLabel} Submissions</h2>
            <div className="flex items-center gap-1.5 bg-gray-50 rounded-lg p-1">
              {availableChartOptions.map(o => (
                <button key={o.key} onClick={() => setChartType(o.key)}
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          <div className="px-5 py-3.5 border-b border-gray-100">
            <h2 className="text-sm font-semibold text-black">Submissions Overview</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="text-left px-5 py-3 font-semibold text-neutral-600">Type</th>
                  <th className="text-right px-5 py-3 font-semibold text-neutral-600">Count</th>
                  <th className="text-right px-5 py-3 font-semibold text-neutral-600">Share</th>
                </tr>
              </thead>
              <tbody>
                {rangeData.byType.map((s, i) => {
                  ...
                  return (
                    <tr key={s.name} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-5 py-3 text-right font-semibold">{s.value}</td>
              ...
              <tfoot>
                <tr className="bg-gray-50">
                  ...
                  <td className="px-5 py-3 text-right font-semibold text-black">{rangeData.total}</td>
                  <td className="px-5 py-3 text-right text-neutral-500">100%</td>
                </tr>
              </tfoot>
            </table>
          </div>
