...
const LUCIDE_ICON_NAMES = Object.keys(LuIcons)
  .filter((k) => k.startsWith("Lu"))
  .map((k) => k.slice(2))
  .sort();
...
const SECTION_TYPES = [
  { value: "text", label: "Text", desc: "Heading + body paragraphs" },
  {
    value: "text_stats",
    label: "Text + Stats",
    desc: "Heading, paragraphs, and stat cards combined",
  },
  {
    value: "stats_row",
    label: "Stats Row",
    desc: "Number + label cards (4-column)",
  },
  {
    value: "feature_grid",
    label: "Feature Grid",
    desc: "Icon + title + description cards (4-column)",
  },
  {
    value: "cta",
    label: "CTA Banner",
    desc: "Call-to-action banner with button link",
  },
];
...
function ImageUploader({ value, onChange, label }) {
  ...
  return (
    <div>
      ...
      {uploadError && (
        <p className="text-xs text-destructive-500 mt-1">{uploadError}</p>
      )}
      {value && (
        <img
          src={value}
          alt=""
          className="mt-2 h-28 w-full object-cover rounded-lg border border-admin-200"
        />
      )}
    </div>
  );
...
function IconPicker({ value, onChange }) {
  ...
  const filtered = useMemo(() => {
    ...
    return LUCIDE_ICON_NAMES.filter((n) => n.toLowerCase().includes(q)).slice(
      0,
      100,
    );
  }, [search]);
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        {IconComp ? (
          <IconComp className="w-5 h-5 text-admin-500" />
        ) : (
          <div className="w-5 h-5" />
        )}
        <span className={`flex-1 ${value ? "" : "text-admin-400"}`}>
          {value || "Select an icon..."}
        </span>
        <FiChevronRight
          className={`w-4 h-4 text-admin-400 transition-transform ${open ? "rotate-90" : ""}`}
        />
      </button>
      {!isValid && value && (
        <p className="text-xs text-destructive-500 mt-1">
          Invalid icon name — must be a valid Lucide icon
        </p>
      )}
      {open && (
        <div className="absolute z-50 top-full mt-1 left-0 right-0 bg-white border border-admin-200 rounded-lg shadow-lg max-h-64 overflow-hidden flex flex-col">
          ...
          <div className="overflow-y-auto admin-scrollbar flex-1">
            {filtered.length === 0 ? (
              <p className="p-3 text-sm text-neutral-400 text-center">
                No icons found
              </p>
            ) : (
              filtered.map((name) => {
...
function ReorderableList({ items, onChange, renderItem, getKey }) {
  ...
  return items.map((item, i) => (
    <div
      ...
      className="border border-admin-200 rounded-lg p-4"
    >
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            onClick={() => move(i, -1)}
            disabled={i === 0}
            className="p-1 text-emerald-500 hover:text-emerald-700 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
          >
          ...
          <button
            ...
            className="p-1 text-cyan-500 hover:text-cyan-700 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
          >
      ...
      const blocks = Array.isArray(section.content)
        ...
        : (() => {
            if (typeof section.content === "string") {
              try {
                const p = JSON.parse(section.content);
            ...
            const list = [];
            if (section.heading)
              list.push({ type: "heading", text: section.heading });
      ...
      return (
        <div className="space-y-6">
          <ImageUploader
            ...
            label="Side Image (optional — shows on the right)"
          />
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-neutral-400 flex items-center gap-1">
                <FiMove className="w-3 h-3" /> Drag to fix positions
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <AlignButtons
                value={section.contentAlign || "center"}
            ...
            {blocks.map((b, i) => (
              <div key={i} className="flex items-start gap-2 p-3 bg-white">
                <div className="mt-1 flex flex-col items-center gap-1">
                  <span
                    className="text-neutral-300 hover:text-admin-500 cursor-grab active:cursor-grabbing"
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1.5">
                    <span
                      className={`text-[10px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded ${b.type === "heading" ? "bg-blue-50 text-blue-600" : "bg-neutral-100 text-neutral-500"}`}
                    >
                    ...
                    <button
                      ...
                      onClick={() =>
                        updateBlock(i, {
                          type: b.type === "heading" ? "paragraph" : "heading",
                        })
                      }
        ...
        <div className="space-y-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <AlignButtons
                value={section.headingAlign || "center"}
              ...
              renderItem={(item, i, onItemChange) => (
                <div className="space-y-4">
                  ...
                  <ImageUploader
                    ...
                    onChange={(v) => onItemChange({ ...item, image_url: v })}
                    label="Photo"
                  />
                </div>
              )}
          ...
          <div className="space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <AlignButtons
                  value={section.subheadingAlign || "center"}
                  onChange={(v) => set({ subheadingAlign: v })}
                />
              </div>
            </div>
          </div>
...
const SECTION_TYPE_LABELS = Object.fromEntries(
...
export default function AboutPageEditor() {
  ...
  function removeSection(idx) {
    ...
    setExpandedIdx((prev) =>
      prev === idx ? null : prev > idx ? prev - 1 : prev,
    );
  }
  ...
  function validate() {
    ...
    sections.forEach((sec, i) => {
      if (
        sec.section_type === "feature_grid" ||
        sec.section_type === "text_stats"
      ) {
        (sec.items || []).forEach((item, j) => {
          if (item.icon && !LUCIDE_ICON_NAMES.includes(item.icon)) {
            errors.push(
              `Section ${i + 1} (${SECTION_TYPE_LABELS[sec.section_type] || sec.section_type}), Item ${j + 1}: "${item.icon}" is not a valid Lucide icon name`,
            );
          }
        });
      }
  ...
  async function handleSave(e) {
    ...
    setSaving(true);
    setSaveError("");
    if (!navItemId && !navItemIdRef.current) {
      setSaveError("No nav item linked — please refresh and try again");
      setSaving(false);
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <ImageUploader
                  ...
                  onChange={(v) => setHero({ ...hero, hero_image: v })}
                  label="Hero Image"
                />
                {hero.hero_image && (
                  <button
                    ...
                    className="text-xs text-destructive-500 hover:text-destructive-700 cursor-pointer"
                  >
                    Remove image
                  </button>
                )}
            ...
            <div className="space-y-6">
              ...
              {sections.length === 0 && (
                <p className="text-sm text-neutral-400 text-center py-8">
                  No sections yet. Select a type above and click "Add".
                </p>
              )}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          ...
                          {sec.heading && (
                            <span className="text-xs text-neutral-400 max-w-48 truncate">
                              — {sec.heading}
                            </span>
                          )}
                          {sec.hidden && (
