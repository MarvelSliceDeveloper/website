...
function ListEditor({ items, onChange, fields, labelKey = "label" }) {
  ...
  const updateItem = (i, key, value) =>
    onChange(
      items.map((item, j) => (j === i ? { ...item, [key]: value } : item)),
    );
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-wrap gap-6 pt-2">
                  <label className="flex items-center gap-2 text-sm cursor-pointer">
                    <input
                      type="checkbox"
                      checked={t.certificate}
                      onChange={(e) => u("certificate", e.target.checked)}
                      className="rounded"
                    />{" "}
                    Certificate
                  </label>
                  ...
                  <label className="flex items-center gap-2 text-sm cursor-pointer">
                    <input
                      type="checkbox"
                      checked={t.popular}
                      onChange={(e) => u("popular", e.target.checked)}
                      className="rounded"
                    />{" "}
                    Popular
                  </label>
                  <label className="flex items-center gap-2 text-sm cursor-pointer">
                    <input
                      type="checkbox"
                      checked={t.trending}
                      onChange={(e) => u("trending", e.target.checked)}
                      className="rounded"
                    />{" "}
                    Trending
                  </label>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {t.trainingModules.map((mod, i) => (
                      <div
                        key={i}
                        className="border border-admin-200 rounded-lg p-4 bg-white"
                      >
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <button
                            onClick={() =>
                              u(
                                "trainingModules",
                                t.trainingModules.filter((_, j) => j !== i),
                              )
                            }
