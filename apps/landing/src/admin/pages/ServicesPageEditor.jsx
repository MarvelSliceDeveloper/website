...
export default function ServicesPageEditor() {
  ...
  const [journeySubheading, setJourneySubheading] = useState(
    ...
    async function resolve() {
      ...
      if (item?.id) {
        ...
        if (page) {
          ...
          if (cardsSec?.items)
            setServices(
              cardsSec.items.map((i) =>
                typeof i === "string"
                  ? { title: i, description: "", icon: "" }
                  : { ...i, icon: i.icon || "" },
              ),
            );
          ...
          if (timelineSec) {
            ...
            setJourneySubheading(
              timelineSec.subheading ||
                "A structured path from enrollment to career success.",
            );
            if (timelineSec.items?.length) setSteps(timelineSec.items);
            else setSteps(DEFAULT_STEPS);
            if (timelineSec.features?.length)
              setJourneyFeatures(timelineSec.features);
  ...
  async function handleSave(e) {
    ...
    const validate = (items, label) => {
      for (let i = 0; i < items.length; i++) {
        ...
        if ((item.title || "").length > 50)
          return `${label} ${i + 1}: title exceeds 50 characters.`;
        if (wordCount(item.description) > 40)
          return `${label} ${i + 1}: description exceeds 40 words.`;
        if (item.number !== undefined && (item.number || "").length > 2)
          return `${label} ${i + 1}: step number exceeds 2 characters.`;
        if (item.colorHex && !/^#[0-9a-fA-F]{6}$/.test(item.colorHex))
          return `${label} ${i + 1}: color must be a valid hex (#RRGGBB).`;
      }
      return null;
    };
    const validationError =
      validate(services, "Service") || validate(steps, "Step");
    if (validationError) {
    ...
    const sections = [
      services.length > 0
        ? { section_type: "cards", heading: "What We Offer", items: services }
        : null,
      steps.length > 0
        ? {
    ...
    if (!navItemId && !navItemIdRef.current) {
      setSaveError("No nav item linked");
      setSaving(false);
      savingRef.current = false;
      return;
    }
            ...
            <div className="space-y-8">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Your Learning Journey"
                      className={inputClass}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text"
                      value={journeySubheading}
                      onChange={(e) => setJourneySubheading(e.target.value)}
                      placeholder="A structured path from enrollment to career success."
                      className={inputClass}
                    />
                  </div>
                </div>
                    ...
                    <div className="space-y-4">
                      <div className="grid sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input
                            type="text"
                            value={s.number || String(i + 1).padStart(2, "0")}
                            onChange={(e) => {
                              const u = [...steps];
              ...
              <RepeatableItemList
                ...
                onAdd={() =>
                  setJourneyFeatures([
                    ...journeyFeatures,
                    { title: "", description: "", icon: "ClipboardCheck" },
                  ])
                }
                ...
                renderItem={(f, i) => (
                  <RepeatableItemCard
                    ...
                    onRemove={() =>
                      setJourneyFeatures(
                        journeyFeatures.filter((_, j) => j !== i),
                      )
                    }
                  >
                    <div className="space-y-4">
                      ...
                      <div className="grid sm:grid-cols-2 gap-4">
                        <IconPicker
                          value={f.icon}
                          onChange={(v) => {
                            const u = [...journeyFeatures];
                            u[i] = { ...u[i], icon: v };
                            setJourneyFeatures(u);
                          }}
                        />
                        <div>
                          <label className="block text-xs font-semibold text-neutral-700 mb-1.5 uppercase tracking-wider">
