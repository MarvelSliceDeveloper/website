    ...
    <div>
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 px-3 py-2 border border-admin-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-admin-500 focus:border-transparent transition-all"
...
export default function TrainingPageEditor() {
  ...
  const { dirty, reset } = useDirty(
    [hero, programs, features, cta, faqs],
    loading,
  );
    ...
    async function resolve() {
      ...
      if (item?.id) {
        ...
        if (page) {
          ...
          if (featuresSec?.items)
            setFeatures(
              featuresSec.items.map((i) =>
          ...
          if (ctaSec)
            setCta({
              heading: ctaSec.heading || "",
              content: ctaSec.content || "",
              link: ctaSec.image_url || "",
            });
  ...
  async function handleSave(e) {
    ...
    const sections = [
      ...
      features.length > 0
        ...
        : null,
      cta.heading || cta.content
        ? {
            section_type: "cta",
            heading: cta.heading,
            content: cta.content,
            image_url: cta.link || null,
          }
        : null,
      faqs.length > 0
        ? {
              ...
              <RepeatableItemList
                ...
                renderItem={(p, i) => (
                  <RepeatableItemCard
                    ...
                    label="Program"
                    onRemove={() =>
                      setPrograms(programs.filter((_, j) => j !== i))
                    }
                  >
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={cta.content}
                    onChange={(e) =>
                      setCta({ ...cta, content: e.target.value })
                    }
                    className={inputClass}
                  />
                </div>
