      ...
      if (!isNew) {
        const [courseRes, tabsRes, faqsRes, tagsRes] = await Promise.all([
          supabase
            .from("courses")
            .select(
              `*, highlights(*), overview_faqs(*), projects(*), certifications(*)`,
            )
            .eq("id", id)
            .single(),
          supabase
        ...
        if (courseRes.data && !courseRes.error) {
          setCourse((p) => ({
            ...
            ...courseRes.data,
            status:
              courseRes.data.status === "Inactive" ||
              courseRes.data.status === "Unpublished"
                ? "Draft"
                : courseRes.data.status,
            is_published:
              courseRes.data.status === "Inactive" ||
              courseRes.data.status === "Unpublished" ||
              courseRes.data.status === "Draft"
                ? false
                : !!courseRes.data.is_published,
            tabs: tabsRes.data || [],
            faqs: faqsRes.data || [],
          }));
        }
  ...
  async function handleSave() {
    ...
    if (course.status !== "Draft") {
      if ((course.highlights || []).length < 9) {
        setTab("highlights");
        setMessage(
          `Cannot save course: Minimum 9 Key Highlights are required for ${course.status} status. Currently configured: ${(course.highlights || []).length}/9.`,
        );
        setSaveError(
          `Minimum 9 Key Highlights are required (currently ${(course.highlights || []).length}/9). Use preset buttons to add 9, 12, or 15 items.`,
        );
        isSavingRef.current = false;
        setSaving(false);
        return;
      ...
      if ((course.projects || []).length !== 3) {
        setTab("projects");
        setMessage(
          `Cannot save course: Exactly 3 Projects are required for ${course.status} status. Currently configured: ${(course.projects || []).length}/3.`,
        );
        setSaveError(
          `Exactly 3 Projects are required (currently ${(course.projects || []).length}/3). Please configure exactly 3 projects.`,
        );
        isSavingRef.current = false;
        setSaving(false);
        return;
      ...
      if ((course.faqs || []).length > 4) {
        setTab("faqs");
        setMessage(
          `Cannot save course: Maximum 4 General FAQs allowed. Currently configured: ${(course.faqs || []).length}/4.`,
        );
        setSaveError(
          `Maximum 4 General FAQs allowed (currently ${(course.faqs || []).length}/4). Please remove extra FAQs.`,
        );
        isSavingRef.current = false;
        setSaving(false);
        return;
    ...
    if (course.status === "Coming Soon" && !course.start_date) {
      ...
      setSaveError(
        'Start date and time is required for "Coming Soon" courses.',
      );
      isSavingRef.current = false;
      setSaving(false);
      return;
    ...
    try {
      ...
      const payload = {
        ...
        start_date: course.start_date
          ? fromDateTimeLocal(course.start_date)
          : null,
      ...
      if (isNew) {
        ...
        if (course.certifications.length > 0) {
          const cleanCert = course.certifications.map((c) => {
            ...
            return {
              ...rest,
              recognized_companies: (rest.recognized_companies || []).filter(
                Boolean,
              ),
              course_id: id,
            };
    ...
    >
      {message && (
        <div className="fixed top-6 right-6 z-50 flex flex-col gap-3 pointer-events-none">
          <div
            className={`p-4 rounded-xl flex items-center gap-3 text-sm shadow-xl animate-fade-in-up pointer-events-auto min-w-[300px] max-w-[calc(100vw-2rem)] ${
      ...
      <div className="flex flex-col lg:flex-row gap-[15px] items-start">
        <div className="hidden lg:block transition-all duration-200 lg:w-[220px] lg:shrink-0">
          <nav className="sticky top-6 self-start max-h-[calc(100vh-80px)] overflow-visible">
            <div
              className="bg-white rounded-xl flex flex-col overflow-visible border border-gray-300"
                ...
                return (
                  <button
                    ...
                    className={`relative w-full flex items-center gap-2.5 text-left px-4 py-3 border-b border-gray-200 last:border-b-0 focus:outline-none transition-colors ${
                      ...
                      isActive
                        ? "bg-admin-600 text-white shadow-md z-10"
        ...
        <div className="flex-1 min-w-0">
          <SectionSelect
            items={editorTabs.map((t) => ({ key: t, label: tabMeta[t].label }))}
            value={tab}
            onChange={setTab}
            label="Section"
          />
          ...
          >
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    >
                      ...
                      /35 words | {(course.description || "").length}/300 chars
                    </span>
                  </div>
                  <textarea
                    ...
                    onChange={(e) =>
                      update(
                        "description",
                        limitDescriptionText(e.target.value),
                      )
                    }
                    maxLength={300}
                    rows={4}
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs font-semibold text-neutral-400">
                          {(course.checklist_items || []).slice(0, 6).length}/6
                          items max
                        </span>
                      </div>
                      <textarea
                        value={(course.checklist_items || [])
                  ...
                  <div>
                    ...
                    {course.status === "Coming Soon" ? (
                      <p className="text-xs text-neutral-500 mt-1.5">
                        ...
                        launch countdown. It becomes Active automatically once
                        the start date arrives.
                      </p>
                    ) : (
                      <p className="text-xs text-neutral-400 mt-1.5">
                        Selecting "Coming Soon" lists this course in the home
                        page Upcoming Courses section with a launch countdown.
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="border-t border-admin-200 pt-6 mt-6">
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <textarea
                        value={course.cta_description || ""}
                        onChange={(e) =>
                          update("cta_description", e.target.value)
                        }
                        rows={2}
                        className="w-full px-3 py-2.5 border border-admin-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-admin-500/20 transition-all"
                    ...
                    <div>
                      <ImageUploader
                        value={course.cta_background_image || ""}
                        onChange={(v) => update("cta_background_image", v)}
            ...
            {tab === "tabs" && !isNew && (
              <div className="space-y-6">
                {(() => {
                  const contentTabs = course.tabs.filter(
                    (t) =>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {course.highlights.length < 9 ? (
                      ...
                      <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200">
                        ✓ Grid Ready ({course.highlights.length} items ={" "}
                        {Math.ceil(course.highlights.length / 3)} rows)
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
