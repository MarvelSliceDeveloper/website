...
export default function UpcomingCoursesManager() {
  ...
  const isAllSelected =
    classes.length > 0 && classes.every((c) => selectedIds.includes(c.id));
  ...
  async function handleBulkDelete() {
    ...
    if (
      !(await confirm(
        `Delete ${count} selected upcoming class${count > 1 ? "es" : ""}? This cannot be undone.`,
      ))
    )
      return;
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <span className="font-semibold text-black">{row.course_name}</span>
      ),
    },
    {
      header: "Batch",
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex items-center justify-end gap-1.5">
          <button
            onClick={() => navigate(`/admin/upcoming-courses/${row.id}`)}
