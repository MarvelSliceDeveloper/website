  ...
  return (
    <div className="bg-white rounded-lg border border-admin-200 overflow-hidden">
      <div className="flex items-center gap-3 px-5 py-3.5 bg-white/80 border-b border-admin-100">
        ...
        >
          {expanded ? (
            <FiChevronDown className="w-4 h-4 text-admin-400" />
          ) : (
            <FiChevronRight className="w-4 h-4 text-admin-400" />
          )}
        </button>
        <input
          type="text"
...
export default function FooterManager() {
  ...
  async function deleteColumn(idx) {
    const col = columns[idx];
    if (!(await confirm(`Delete column "${col.title}" and all its links?`)))
      return;
  ...
  async function addLink(colIdx) {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        label: "New Link",
        url: "#",
        sort_order: (col.footer_links || []).length,
      })
      .select()
      .single();
    if (!error) {
      ...
      newCols[colIdx] = {
        ...col,
        footer_links: [...(col.footer_links || []), data],
      };
      setColumns(newCols);
    }
    queryClient.invalidateQueries({ queryKey: ["footer"] });
  ...
  async function deleteLink(colIdx, linkIdx) {
    ...
    newCols[colIdx] = {
      ...col,
      footer_links: col.footer_links.filter((_, i) => i !== linkIdx),
    };
    setColumns(newCols);
  }
      ...
      {columns.length === 0 ? (
        <div className="bg-white rounded-lg border border-admin-200 p-12 text-center">
          <p className="text-sm text-neutral-400">
            No footer columns yet. Click "Add Column" to get started.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {columns.map((col, i) => (
            <ColumnCard
              ...
              onAddLink={() => addLink(i)}
              onUpdateLink={(li, field, value) =>
                updateLink(i, li, field, value)
              }
