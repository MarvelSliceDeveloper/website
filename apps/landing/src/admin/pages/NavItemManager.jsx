...
export default function NavItemManager() {
  ...
  const sections = [
    ...new Set(items.filter((i) => i.parent_label).map((i) => i.parent_label)),
  ].sort();
  ...
  return (
    <PageShell backTo="/admin" title="Nav Item Manager">
      {sections.length === 0 ? (
        <div className="text-center py-16 text-neutral-400 text-sm border rounded-xl bg-white">
          No nav items found.
        </div>
      ) : (
        <div className="space-y-6">
          {sections.map((section) => {
            const sectionItems = items.filter(
              (i) => i.parent_label === section,
            );
            return (
              <div
                key={section}
                className="rounded-xl border border-admin-200 bg-white overflow-hidden"
              >
                    ...
                    return (
                      <div key={item.id} className="px-5 py-3">
                        <div className="flex items-center gap-3 flex-wrap">
                          ...
                          {item.path && (
                            <span className="text-[11px] text-neutral-400 bg-white px-2 py-0.5 rounded-full truncate max-w-[120px] hidden sm:inline">
                              {item.path}
                            </span>
                          )}
                          <span
                          ...
                          <Link
                            to={`/admin/nav-menu/children/${item.id}`}
                          ...
                          <div
                            ...
                            ref={
                              courseDropdown === item.id ? dropdownRef : null
                            }
                          >
                            <button
                              onClick={() =>
                                setCourseDropdown(
                                  courseDropdown === item.id ? null : item.id,
                                )
                              }
                            ...
                            >
                              <FiBookOpen className="w-3.5 h-3.5" /> Linked
                              Courses
                            </button>
                              ...
                              <div className="absolute top-full right-0 mt-1 bg-white border border-admin-200 rounded-lg shadow-lg z-50 min-w-[220px] max-h-[280px] flex flex-col">
                                <div className="overflow-y-auto admin-scrollbar">
                                  {courses.length === 0 ? (
                                    ...
                                    courses.map((c) => {
                                      const checked = itemCourses.some(
                                        (lc) => lc.id === c.id,
                                      );
                                      return (
                                        <label
                                        ...
                                        >
                                          <div
                                            className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors ${checked ? "bg-admin-600 border-admin-600" : "border-admin-200"}`}
                                          >
                                            {checked && (
                          ...
                          <div className="ml-5 mt-2 space-y-1">
                            {children.map((child) => {
                              ...
                              return (
                                <div
                                  key={child.id}
                                  className="flex items-center gap-2 text-xs text-neutral-500 pl-3 border-l-2 border-admin-200 py-1"
                                >
