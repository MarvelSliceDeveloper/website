...
import {
  FiBookOpen, FiUsers, FiFileText, FiMessageCircle,
  FiPlusCircle, FiInbox, FiMenu, FiBriefcase, FiBarChart2,
} from 'react-icons/fi';
...
function StatCard({ icon: Icon, label, value, link, iconTone = 'slate' }) {
  ...
  return (
    <Wrapper to={link} className="block bg-white rounded-lg border border-admin-200 p-3 hover:shadow-elevated hover:border-admin-300 transition-all group" style={{ boxShadow: 'rgba(100, 100, 111, 0.15) 0px 4px 14px 0px' }}>
      <div className="flex items-center justify-between gap-2">
        <div className={`w-8 h-8 flex items-center justify-center rounded-md ${toneClasses[iconTone] || toneClasses.slate}`}>
          <Icon className="w-3.5 h-3.5" />
        </div>
        <span className="text-lg font-semibold tabular-nums text-black leading-none">{value}</span>
      </div>
      <p className="text-[0.85rem] font-medium text-neutral-500 mt-2 truncate">{label}</p>
    </Wrapper>
  );
}
...
const quickLinks = [
  { to: '/admin/courses/wizard', label: 'Add Course', icon: FiPlusCircle, tone: 'emerald' },
  { to: '/admin/blog/new', label: 'Add Blog', icon: FiFileText, tone: 'fuchsia' },
  { to: '/admin/nav-menu?section=Software%20Learning&tab=add', label: 'Add Menu', icon: FiMenu, tone: 'cyan' },
  { to: '/admin/jobs/new', label: 'Add Job', icon: FiBriefcase, tone: 'amber' },
  { to: '/admin/internships/new', label: 'Add Intern', icon: FiUsers, tone: 'teal' },
...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      const results = await Promise.allSettled([
        ...
        supabase.from('about_submissions').select('*', { count: 'exact', head: true }),
        supabase.from('conversations').select('*', { count: 'exact', head: true }),
      ]);
      const getCount = (r) => (r.status === 'fulfilled' ? r.value.count ?? 0 : 0);
      setStats({
        courses: getCount(results[0]), companies: getCount(results[1]), blogPosts: getCount(results[2]),
      });
      setPending({
        career: getCount(results[5]), careerContact: getCount(results[6]), contact: getCount(results[7]), brochure: getCount(results[8]), form: getCount(results[9]), newsletter: getCount(results[10]), about: getCount(results[11]), chat: getCount(results[12]),
      });

      const courses = results[3].status === 'fulfilled' ? (results[3].value.data ?? []) : [];
      const blogs = results[4].status === 'fulfilled' ? (results[4].value.data ?? []) : [];
  ...
  const mainCards = [
    ...
    { label: 'Live Chat', value: pending.chat, icon: FiMessageCircle, link: '/admin/chats?tab=live', subtitle: 'Live conversations', iconTone: 'emerald' },
  ...
  const submissionCards = [
    ...
    { label: 'Newsletter Subscribers', value: pending.newsletter, icon: FiInbox, link: '/admin/newsletter-subscribers', subtitle: 'Subscribers', iconTone: 'orange' },
    { label: 'About Submissions', value: pending.about, icon: FiInbox, link: '/admin/about-submissions', subtitle: 'Submissions', iconTone: 'indigo' },
    { label: 'Chat Submissions', value: pending.chat, icon: FiMessageCircle, link: '/admin/chat-submissions', subtitle: 'Submissions', iconTone: 'red' },
  ];

  return (
    <PageShell hideBorder>
      <h2 className="text-sm font-semibold text-neutral-700 uppercase tracking-wider mb-2">Overview</h2>
      ...
      <h2 className="text-sm font-semibold text-neutral-700 uppercase tracking-wider mt-5 mb-2">Form Responses</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1">
          <h2 className="text-sm font-semibold text-neutral-700 uppercase tracking-wider mb-3">Quick Links</h2>
        ...
        <div className="lg:col-span-2">
          <h2 className="text-sm font-semibold text-neutral-700 uppercase tracking-wider mb-3">Recent Activity</h2>
          <Card>
