...
export default function NavPageEditor() {
  ...
  const { dirty, reset } = useDirty(
    [heading, subheading, heroImage, sections],
    isLoading,
  );
  ...
  useEffect(() => {
    fetchLinkedCourses();
    fetchAllCourses();
  }, [fetchLinkedCourses, fetchAllCourses]);
  ...
  async function handleSave(e) {
    ...
    if (courseChanges) {
      ...
      const toRemove = [...currentIds].filter(
        (cid) => !linkedCourseIds.has(cid),
      );
      const updates = [];
      toAdd.forEach((cid) =>
        updates.push(
          supabase.from("courses").update({ nav_item_id: id }).eq("id", cid),
        ),
      );
      toRemove.forEach((cid) =>
        updates.push(
          supabase.from("courses").update({ nav_item_id: null }).eq("id", cid),
        ),
      );
  ...
  return (
    <PageShell backTo="/admin/nav-menu" title="">
      <SaveBar
        saving={saving}
        saved={saved}
        saveError={saveError}
      ...
      <form onSubmit={handleSave}>
        ...
        <SectionAccordion title="Content Sections" defaultExpanded={false}>
          ...
          <div className="space-y-3">
            {sections.map((sec, idx) => (
              <div
                key={idx}
                className="border border-admin-200 rounded-lg p-5 hover:border-admin-200 transition-colors"
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateSection(idx, "section_type", e.target.value)
                      }
        ...
        <SectionAccordion title="Linked Courses" defaultExpanded={false}>
          ...
          <p className="text-sm text-neutral-500 mb-4">
            Select courses to display on this page.
          </p>
                ...
                .map((course) => (
                  <label
                    ...
                    className={`flex items-center gap-3 p-2.5 rounded-lg cursor-pointer transition-colors ${
                      linkedCourseIds.has(course.id)
                        ? "bg-white border border-admin-200"
                  ...
                  >
                    ...
                    >
                      ...
                      <input
                        type="checkbox"
                        checked={linkedCourseIds.has(course.id)}
                    ...
                    <Link
                      ...
                      className="text-sm text-admin-900 font-medium hover:text-admin-600 truncate"
