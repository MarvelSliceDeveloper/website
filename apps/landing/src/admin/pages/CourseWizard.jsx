...
export default function CourseWizard() {
  ...
  const rawReturn =
    ...
    (searchParams.get("category")
      ? `/admin/courses?category=${encodeURIComponent(searchParams.get("category"))}`
      : null);
  const returnUrl = rawReturn
  ...
  const [filterSection, setFilterSection] = useState(
    searchParams.get("category") || "Software Learning",
  );
  const initialStatus =
    searchParams.get("status") === "coming-soon" ? "Coming Soon" : "Active";
  ...
  function getMissingFields() {
    ...
    if (!(c.checklist_items || []).join("").trim())
      add(STEPS[1].label, "What You'll Learn");
    if (c.highlights.length < 9 || c.highlights.some((h) => !h.label.trim()))
      add(
        STEPS[2].label,
        `Key Highlights (Minimum 9 required — currently ${c.highlights.length}/9)`,
      );
    if (c.projects.length !== 3 || c.projects.some((p) => !p.title.trim()))
      add(
        STEPS[2].label,
        `Projects (Exactly 3 required — currently ${c.projects.length}/3)`,
      );
    if (
      c.faqs.length === 0 ||
      c.faqs.length > 4 ||
      c.faqs.some((f) => !f.question.trim())
    )
      add(
        STEPS[3].label,
        `General FAQs (${c.faqs.length > 4 ? `Maximum 4 allowed — currently ${c.faqs.length}/4` : "Required"})`,
      );
  ...
  async function handleSave() {
    ...
    if (c.status === "Coming Soon" && !c.start_date) {
      ...
      setStep(0);
      setMessage(
        'Please set the start date and time — it is required for "Coming Soon" courses.',
      );
    ...
    try {
      ...
      if (courseTags.length > 0)
        inserts.push(
          supabase
            .from("course_tags")
            .insert(courseTags.map((tid) => ({ course_id: cid, tag_id: tid }))),
        );
  ...
  return (
    ...
    >
      <div className="-mt-4">
        <FolderTabs
          tabs={STEPS.map((s, i) => ({ id: i, title: s.label }))}
          activeTab={step}
          onChange={(id) => handleStepClick(id)}
        />
          ...
          {step === 0 && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-start">
                {filterSection &&
                  (() => {
                    const topLevel = availablePaths.filter(
                      (p) => p.parent_label === filterSection && !p.parent_id,
                    );
                    ...
                    return (
                      <>
                        ...
                        {catL1 &&
                          kids1.length > 0 &&
                          dd(
                            `Sub-topic under ${findItem(catL1)?.label || ""}`,
                            kids1,
                            catL2,
                            setCatL2,
                          )}
                        {catL2 &&
                          kids2.length > 0 &&
                          dd(
                            `Sub-topic under ${findItem(catL2)?.label || ""}`,
                            kids2,
                            catL3,
                            setCatL3,
                          )}
                ...
                <div>
                  <label className="block text-sm font-semibold text-black mb-1">
                    Course Title <span className="text-destructive-500">*</span>
                  </label>
                  <input
                    value={c.title}
                ...
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span
                    className={`text-xs font-semibold ${
                      ...
                      (c.description || "").trim().split(/\s+/).filter(Boolean)
                        .length >= 35
                  ...
                  >
                    ...
                    /35 words | {(c.description || "").length}/300 chars
                  </span>
                </div>
                <textarea
                  value={c.description || ""}
                  onChange={(e) =>
                    u("description", limitDescriptionText(e.target.value))
                  }
                  maxLength={300}
                  rows={4}
                  required
                ...
                <div>
                  ...
                  {c.status === "Coming Soon" && (
                    <p className="text-xs text-neutral-500 mt-1.5">
                      Listed in the home page Upcoming Courses section with a
                      launch countdown. It becomes Active automatically once the
                      start date arrives.
                    </p>
                  )}
                </div>
              ...
              {c.status === "Coming Soon" && (
                <div>
                  ...
                  />
                  {startDateError && (
                    <p className="text-xs text-destructive-500 mt-1.5">
                      Please set the start date and time for this Coming Soon
                      course.
                    </p>
                  )}
                  {!startDateError && (
                    <p className="text-xs text-neutral-500 mt-1.5">
                      The course becomes Active automatically once this date
                      arrives.
                    </p>
                  )}
                </div>
          ...
          {step === 1 && (
            <div className="space-y-6">
              <h2 className="text-lg font-semibold text-black flex items-center gap-2">
                <FiMonitor className="w-5 h-5 text-cyan-600" /> Media
              </h2>
              <div className="grid sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <ImageUploader
                    ...
                    onChange={(url) => u("hero_image_url", url)}
                  />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-black mb-1">
                  ...
                  <ImageUploader
                    value={c.video_thumbnail_url}
                    onChange={(url) => u("video_thumbnail_url", url)}
                  />
                </div>
              </div>
              <div>
                <label className="block text-sm font-semibold text-black mb-1">
                  Course Video (YouTube URL){" "}
                  <span className="text-destructive-500">*</span>
                </label>
                <input
                  value={c.video_url || ""}
              ...
              <h2 className="text-lg font-semibold text-black flex items-center gap-2">
                <FiBookOpen className="w-5 h-5 text-violet-600" /> Content
              </h2>
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-black mb-1">
                    CTA Left <span className="text-destructive-500">*</span>
                  </label>
                  <input
                    value={c.cta_left || ""}
                    onChange={(e) => u("cta_left", e.target.value)}
                ...
                <div>
                  <label className="block text-sm font-semibold text-black mb-1">
                    CTA Right <span className="text-destructive-500">*</span>
                  </label>
                  <input
                    value={c.cta_right || ""}
                    onChange={(e) => u("cta_right", e.target.value)}
                    className="w-full px-3 py-2.5 border border-admin-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-admin-500/20 text-sm transition-all"
              ...
              <div className="border-t border-admin-200 pt-6 mt-2">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      value={c.cta_heading || ""}
                      onChange={(e) => u("cta_heading", e.target.value)}
                      className="w-full px-3 py-2.5 border border-admin-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-admin-500/20 text-sm transition-all"
                  ...
                  <div>
                    ...
                    <textarea
                      value={c.cta_description || ""}
                      onChange={(e) => u("cta_description", e.target.value)}
                      rows={2}
                      className="w-full px-3 py-2.5 border border-admin-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-admin-500/20 text-sm transition-all"
                  ...
                  <div>
                    <ImageUploader
                      value={c.cta_background_image || ""}
                      onChange={(v) => u("cta_background_image", v)}
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="block text-sm font-semibold text-black">
                    What You'll Learn / Feature Bullet Points (one per line){" "}
                    <span className="text-destructive-500">*</span>{" "}
              ...
              <div>
                {(() => {
                  const contentTabs = c.tabs.filter(
                    (t) =>
                      t.content_type === "overview" ||
                      t.content_type === "syllabus",
                  );
                  const qaCounts = contentTabs.map(
                    (t) => (t.content?.qa || []).length,
                  );
                  ...
                  const isUnbalanced =
                    contentTabs.length > 1 && maxQA > 0 && minQA === 0;
                    ...
                    >
                      <div className="flex items-center justify-between gap-2 mb-1.5 font-bold text-xs uppercase tracking-wider">
                        <div className="flex items-center gap-2">
                          <FiAlertCircle
                            className={`w-4 h-4 shrink-0 ${isUnbalanced ? "text-amber-600" : "text-blue-600"}`}
                          />
                          <span>
                            {isUnbalanced
                              ? "⚠️ Tab Content Unbalanced"
                              : "Tab Content Balance Guidance"}
                          </span>
                        </div>
                        {contentTabs.length > 1 && (
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-extrabold ${isUnbalanced ? "bg-amber-200 text-amber-900" : "bg-blue-200/70 text-blue-900"}`}
                          >
                      ...
                      {isUnbalanced ? (
                        <p className="text-xs leading-relaxed text-amber-800 font-medium">
                          <strong>Attention:</strong> One or more tabs have{" "}
                          {maxQA} Q&A items while another tab has 0 Q&A items.
                          To keep the course page looking balanced and even,
                          please add Q&A items to all tabs or keep item counts
                          matched!
                        </p>
                      ) : (
                        <p className="text-xs leading-relaxed text-blue-800">
                          To ensure tabs look even on the course page, aim for
                          consistent content density across overview & syllabus
                          tabs:
                        </p>
                      )}
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mt-2.5 text-xs font-medium">
                        <div className="bg-white/80 border border-slate-200/80 rounded-lg px-3 py-2 flex items-center gap-1.5">
                          <span
                            className={`w-2 h-2 rounded-full shrink-0 ${isUnbalanced ? "bg-amber-500" : "bg-emerald-500"}`}
                          />
                          <span>
                            <strong>Q&A Items:</strong> 2 – 4 per tab
                          </span>
                        </div>
                        <div className="bg-white/80 border border-slate-200/80 rounded-lg px-3 py-2 flex items-center gap-1.5">
                          <span className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" />
                          <span>
                            <strong>Paragraph Text:</strong> 30 – 80 words
                          </span>
                        </div>
                        <div className="bg-white/80 border border-slate-200/80 rounded-lg px-3 py-2 flex items-center gap-1.5">
                          <span className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" />
                          <span>
                            <strong>First Accordion:</strong> Opens
                            automatically
                          </span>
                        </div>
                      </div>
                ...
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-black">
                    Course Tabs <span className="text-destructive-500">*</span>
                  </h3>
                  <AddButton
                    onClick={() =>
                      u("tabs", [
                ...
                <div className="space-y-4">
                  {c.tabs.map((t, i) => (
                    <div
                      key={i}
                      className="border border-admin-200 rounded-xl p-5 bg-white shadow-xs space-y-4"
                    >
                      <div className="flex items-center justify-between gap-3 pb-3 border-b border-admin-100">
                        <div className="flex items-center gap-3">
                          ...
                          {(() => {
                            const contentTabs = c.tabs.filter(
                              (tab) =>
                                tab.content_type === "overview" ||
                                tab.content_type === "syllabus",
                            );
                            const qaCounts = contentTabs.map(
                              (tab) => (tab.content?.qa || []).length,
                            );
                            const maxQA =
                              qaCounts.length > 0 ? Math.max(...qaCounts) : 0;
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    n[i] = {
                                      ...
                                      content: {
                                        ...n[i].content,
                                        subheadingAlign: align,
                                      },
                                    };
                                    u("tabs", n);
                                  }}
                                  className={`px-2 py-0.5 rounded text-xs font-semibold capitalize transition-all ${
                                    (t.content?.subheadingAlign || "left") ===
                                    align
                                      ? "bg-admin-600 text-white shadow-xs"
                                      : "text-neutral-600 hover:text-neutral-900"
                                  }`}
                        ...
                        <div className="pt-2">
                          ...
                          <div className="space-y-4">
                            {(t.content?.qa || []).map((qa, qi) => (
                              <div
                                key={qi}
                                className="bg-neutral-50/80 border border-admin-200 rounded-xl p-4 space-y-3"
                              >
                                <div className="flex items-center justify-between">
                                  <span className="text-xs font-bold text-admin-600 uppercase tracking-wider">
                                    Feature #{qi + 1}
                                  </span>
                                  <button
                                    onClick={() => {
                                      const n = [...c.tabs];
                                      const qa = n[i].content.qa.filter(
                                        (_, j) => j !== qi,
                                      );
                                      n[i] = {
                                        ...n[i],
                                ...
                                <div>
                                  <div className="flex items-center justify-between mb-1.5">
                                    <label className="block text-xs font-medium text-neutral-600">
                                      Feature Bullets / Answers (one per line)
                                    </label>
            ...
            <div className="space-y-8">
              <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {c.highlights.length < 9 ? (
                      ...
                      <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200">
                        ✓ Grid Ready ({c.highlights.length} items ={" "}
                        {Math.ceil(c.highlights.length / 3)} rows)
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <p className="text-xs text-neutral-500 mb-3">
                  Key Highlights display as a 3-column grid on the course page.
                  Require at least <strong>9 items</strong> (recommended{" "}
                  <strong>9, 12, or 15 items</strong> for balanced rows).
                </p>
                <div className="space-y-3">
                  {c.highlights.map((h, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-3 border border-admin-200 rounded-lg p-3 bg-white"
                    >
                      <span className="text-xs font-bold text-neutral-400 shrink-0 w-6">
                        #{i + 1}
                      </span>
                      <div className="w-32">
                        <IconPicker
                      ...
                      <input
                        ...
                        className="flex-1 px-3 py-2 border border-admin-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-neutral-500/20"
              ...
              <div>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mb-3">
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-semibold text-black">
                      Projects <span className="text-destructive-500">*</span>
                    </h3>
                    {c.projects.length !== 3 ? (
                      <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-100 text-amber-900 border border-amber-300">
                ...
                <p className="text-xs text-neutral-500 mb-3">
                  Each course requires <strong>exactly 3 projects</strong> to
                  display in a balanced 3-column row on the course page.
                </p>
                <div className="space-y-3">
                  {c.projects.map((p, i) => (
                    <div
                      key={i}
                      className="border border-admin-200 rounded-lg p-4 relative bg-white"
                    >
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 border border-admin-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-admin-500/20 mb-2"
              ...
              <div>
                ...
                <div className="space-y-4">
                  {c.certifications.map((cert, i) => (
                    <div
                      key={i}
                      className="border border-admin-200 rounded-lg p-4 relative"
                    >
                      <button
                        onClick={() =>
                          u(
                            "certifications",
                            c.certifications.filter((_, j) => j !== i),
                          )
                        }
            ...
            <div className="space-y-8">
              ...
              <div>
                <h3 className="text-sm font-semibold text-black mb-3">
                  Tags <span className="text-destructive-500">*</span>
                </h3>
                {courseTags.length === 0 && (
                  <p className="text-xs text-destructive-500 mb-2">
                    Select at least one tag to enable saving
                  </p>
                )}
                {allTags.length === 0 ? (
                  ...
                  <div className="relative">
                    ...
                    >
                      {courseTags.length === 0 && (
                        <span className="text-neutral-400">Select tags...</span>
                      )}
                      {courseTags.map((id) => {
                        ...
                        <div className="absolute top-full left-0 right-0 mt-1 max-h-60 overflow-hidden bg-white border border-admin-200 rounded-lg shadow-lg z-20 flex flex-col">
                          ...
                          <div className="max-h-44 overflow-y-auto admin-scrollbar py-1">
                            {allTags.filter((t) =>
                              ...
                              <div className="px-3 py-2.5 text-xs text-neutral-400 italic text-center">
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-admin-200">
              <h2 className="text-lg font-bold text-black flex items-center gap-2">
                <FiAlertCircle className="w-5 h-5 text-destructive-500" />{" "}
                Missing Fields
              </h2>
              <button
