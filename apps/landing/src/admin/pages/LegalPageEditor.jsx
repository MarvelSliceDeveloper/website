...
import {
  ...
  FiTrash2, FiArrowUp, FiArrowDown, FiEdit3,
} from 'react-icons/fi';
...
export default function LegalPageEditor({ pageKey }) {
  ...
  function addSection() {
    setSections(prev => [...prev, { id: uid(), heading: '', body: '', heading_align: 'left', body_align: 'left' }]);
  }
  ...
  async function handleSave(e) {
    ...
    if (pageId) {
      ...
      res = await supabase.from('legal_pages').insert(payload).select('id').single();
    }
  ...
  return (
    <PageShell backTo="/admin" title={label}>
      <SaveBar saving={saving} saved={saved} saveError={saveError} label={label} top />

      <form onSubmit={handleSave} className="bg-white border border-gray-300 rounded-[20px] shadow-sm p-6 space-y-6">
        <div className="grid sm:grid-cols-1 gap-4">
          <div>
            <label className="block text-xs font-semibold text-neutral-700 mb-1.5 uppercase tracking-wider">Page Title</label>
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
              placeholder="Page title shown at the top" className={inputClass} />
          </div>
        </div>

        <div>
          <label className="block text-xs font-semibold text-neutral-700 mb-1.5 uppercase tracking-wider">Intro</label>
          <textarea value={intro} onChange={(e) => setIntro(e.target.value)}
            rows={3} placeholder="Optional intro paragraph below the title" className={inputClass} />
        </div>

        <div>
          <div className="flex items-center justify-between mb-3">
            <label className="block text-xs font-semibold text-neutral-700 uppercase tracking-wider">Sections</label>
          ...
          {sections.length === 0 ? (
            <div className="border-2 border-dashed border-admin-200 rounded-lg p-8 text-center">
              ...
              <p className="text-sm text-neutral-500 mb-4">No sections yet. Add sections with headings and body text.</p>
              <AddButton onClick={addSection} label="Add Section" />
            </div>
          ) : (
            <div className="space-y-4">
              {sections.map((section, index) => (
                <div key={section.id} className="border border-admin-200 rounded-xl p-4 space-y-4">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-xs font-semibold text-admin-500 uppercase tracking-wider">Section {index + 1}</span>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <label className="block text-xs font-semibold text-neutral-700 uppercase tracking-wider">Heading</label>
                      <AlignButtons value={section.heading_align} onChange={(v) => updateSection(section.id, { heading_align: v })} />
                    </div>
                    <input type="text" value={section.heading} onChange={(e) => updateSection(section.id, { heading: e.target.value })}
                      placeholder="Section heading" className={inputClass} />
                  </div>

                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <label className="block text-xs font-semibold text-neutral-700 uppercase tracking-wider">Body</label>
                      <AlignButtons value={section.body_align} onChange={(v) => updateSection(section.id, { body_align: v })} />
                    </div>
                    <textarea value={section.body} onChange={(e) => updateSection(section.id, { body: e.target.value })}
                      rows={6} placeholder="Body text. Use plain text; blank lines create paragraph breaks." className={inputClass} />
                  </div>
                </div>
              ))}
            </div>
