  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        {emptyDescription && (
          <p className="text-xs text-neutral-400 max-w-[200px] w-full">
            {emptyDescription}
          </p>
        )}
    ...
    <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-3 border-b border-admin-100 w-full min-w-0">
      <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 w-full md:w-1/2 min-w-0">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={applySearch}
            className="px-4 py-1.5 h-9 bg-admin-600 text-white text-sm font-medium rounded-lg hover:bg-admin-700 transition-colors"
          >
            Search
          </button>
          {activeSearch && (
            <button
              onClick={clearSearch}
              className="px-3 py-1.5 h-9 text-xs font-medium text-neutral-500 hover:text-neutral-700 transition-colors"
            >
              Clear
            </button>
          )}
  ...
  const renderNoResults = () =>
    filtered.length === 0 && activeSearch ? (
      <div className="flex flex-col items-center py-12 text-center">
        ...
        <button
          onClick={clearSearch}
          className="mt-2 text-xs font-semibold text-neutral-600 hover:text-neutral-700 transition-colors"
        >
          Clear search
        </button>
      </div>
          ...
          {paginated.map((row, rowIndex) => (
            ...
            >
              {columns.map((col, i) => (
                <div
                  key={i}
                  className={`${i === 0 ? "flex-1 min-w-0" : "shrink-0"} ${col.className || ""}`}
                >
    ...
    <div className="bg-white rounded-xl border border-admin-200 shadow-sm overflow-hidden w-full max-w-full min-w-0">
      ...
      <div className="admin-table-scroll w-full max-w-full overflow-x-auto">
        <table className="admin-table min-w-[640px] w-full">
          <thead>
            <tr
              className={`border-b border-admin-100 ${headerRowClass || "bg-brand-blue"}`}
            >
              {columns.map((col, i) => (
                <th
                  key={i}
                  className={`text-left text-xs font-bold uppercase tracking-wider px-4 py-3.5 whitespace-nowrap ${headerCellClass || "text-white"} ${col.className || ""}`}
                >
                  {col.header}
                </th>
              ))}
            ...
            {paginated.map((row, rowIndex) => (
              ...
              >
                {columns.map((col, i) => (
                  <td
                    key={i}
                    className={`px-4 py-3.5 text-sm align-middle text-neutral-700 ${col.className || ""}`}
                  >
                    {renderCell(row, col, (page - 1) * pageSize + rowIndex)}
                  </td>
                ))}
