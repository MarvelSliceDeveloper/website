...
function ReplyModal({ row, onClose, pageTitle }) {
  ...
  if (sent) {
    return (
      ...
      >
        <div
          className="bg-white rounded-xl border border-admin-200 shadow-2xl max-w-lg w-full p-8 text-center cursor-pointer"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="w-12 h-12 rounded-full bg-success-50 flex items-center justify-center mx-auto mb-4">
            <FiCheck className="w-6 h-6 text-success-500" />
          </div>
          <h3 className="text-lg font-semibold text-black mb-1">Reply sent!</h3>
          <p className="text-sm text-neutral-500 mb-6">
            Your email has been sent to {row.email}.
          </p>
          <button
            onClick={onClose}
            className="px-5 py-2 rounded-lg bg-admin-600 text-white text-sm font-medium hover:bg-admin-700 transition-all shadow-sm"
          >
            Done
          </button>
        </div>
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-xl border border-admin-200 shadow-2xl max-w-lg w-full cursor-pointer"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-admin-100">
          <div>
            <h3 className="text-base font-semibold text-black">
              Reply to {row.full_name || row.name || row.email}
            </h3>
        ...
        <div className="p-6 space-y-4">
          ...
          {error && (
            <p className="text-xs text-destructive-500 bg-destructive-50 px-3 py-2 rounded-lg">
              {error}
            </p>
          )}
        </div>
...
function ExportDialog({ type, data, columns, exportFilename, onClose }) {
  ...
  async function handleExport() {
    ...
    try {
      if (type === "csv") {
        ...
        a.href = url;
        a.download = `${exportFilename || "submissions"}-${new Date().toISOString().slice(0, 10)}.csv`;
        a.click();
        ...
        pdf.setFontSize(8);
        pdf.text(
          `Generated: ${new Date().toLocaleDateString()}  |  ${filtered.length} entries`,
          14,
          20,
        );
        pdf.save(
          `${exportFilename || "submissions"}-${new Date().toISOString().slice(0, 10)}.pdf`,
        );
      }
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="relative bg-white rounded-xl border border-admin-200 shadow-2xl w-full max-w-md cursor-pointer"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-admin-100">
          <div>
            ...
            <div className="flex gap-3">
              <div className="flex-1">
                ...
                <input
                  ...
                  className="w-full h-9 px-3 rounded-lg border border-admin-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-neutral-500/20 focus:border-neutral-500 transition-all"
                />
              </div>
              <div className="flex-1">
                <label className="block text-xs font-medium text-neutral-600 mb-1">
...
export default function SubmissionsInbox({
  ...
  async function markRead(row, e) {
    ...
    setData((prev) =>
      prev.map((s) => (s.id === row.id ? { ...s, is_read: true } : s)),
    );
  }
  ...
  async function markUnread(row, e) {
    ...
    setData((prev) =>
      prev.map((s) => (s.id === row.id ? { ...s, is_read: false } : s)),
    );
  }
  ...
  const filtered = useMemo(() => {
    ...
    if (extraFilters) {
      extraFilters.forEach((f) => {
        if (f.apply && f.value !== "all")
          result = result.filter((r) => f.apply(r));
      });
    }
    return result;
  }, [
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-end gap-3 w-full">
            ...
            {hasFilters && (
              <button
                ...
                className="h-9 px-3 text-xs font-medium text-neutral-500 hover:text-neutral-700 transition-colors"
              >
                Clear filters
              </button>
            )}
      ...
      <div className="grid xl:grid-cols-3 gap-4 items-start w-full max-w-full min-w-0">
        <div
          className={`w-full max-w-full min-w-0 ${selected ? "xl:col-span-2" : "xl:col-span-3"}`}
        >
                ...
                <table className="admin-table min-w-[760px] w-full">
                  <thead>
                    <tr className="border-b border-admin-100 bg-brand-blue">
                      <th className="w-10 text-left text-xs font-bold text-white uppercase tracking-wider px-4 py-3.5 whitespace-nowrap">
                        #
                      </th>
                      {columns.map((col, i) => (
                        <th
                          key={i}
                          className={`text-left text-xs font-bold text-white uppercase tracking-wider px-4 py-3.5 whitespace-nowrap ${col.className || ""}`}
                        >
                          {col.header}
                        </th>
                      ))}
                      <th className="text-left text-xs font-bold text-white uppercase tracking-wider px-4 py-3.5 whitespace-nowrap">
                        Actions
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {paged.map((row, idx) => (
                      ...
                      >
                        <td className="px-4 py-3.5 text-xs text-neutral-400 font-mono whitespace-nowrap text-left">
          ...
          <div className="bg-white rounded-xl border border-admin-200 shadow-lg overflow-hidden sticky top-4">
            <div className="flex items-center justify-between px-5 py-4 border-b border-admin-100">
              ...
              <button
                ...
                className="p-1 text-destructive-500 hover:text-destructive-700 rounded-md hover:bg-destructive-50 transition-all"
              >
              ...
              <div className="flex flex-wrap items-center gap-2">
                {!disableReply && (
                  ...
                  >
                    {selected.is_read ? (
                      <>
                        <FiCheck className="w-3.5 h-3.5" /> Read
                      </>
                    ) : (
                      <>
                        <FiMail className="w-3.5 h-3.5" /> Unread
                      </>
                    )}
                  </button>
                )}
                {!disableReply && (
                  ...
                  >
                    {selected.is_read ? (
                      <>
                        <FiMail className="w-3.5 h-3.5" /> Mark as Unread
                      </>
                    ) : (
                      <>
