...
const routes = [
  ...
  {
    label: "Career Enquiry Submissions",
  ...
  {
    label: "Chat History",
    to: "/admin/chats?tab=history",
    icon: FiMessageCircle,
    category: "Settings",
  },
  {
    label: "Social Links",
    to: "/admin/site-settings?section=social",
    icon: FiGlobe,
    category: "Settings",
  },
];
...
export default function CommandPalette({ open, onClose }) {
  ...
  const results = useMemo(() => {
    ...
    return routes
      .map((r) => {
        const score = r.label.toLowerCase().includes(q)
          ? 1
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-start justify-center px-4 pt-[15vh] cursor-pointer"
      onClick={onClose}
    >
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm" />
      <div
        className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl border border-admin-200 overflow-hidden cursor-pointer"
      ...
      >
        <div className="flex items-center gap-3 px-4 border-b border-admin-100">
          ...
          />
          <kbd className="text-[10px] text-neutral-400 bg-admin-100 border border-admin-200 rounded px-1.5 py-0.5 font-mono shrink-0">
            ESC
          </kbd>
        </div>
