  ...
  async function handleExport() {
    ...
    try {
      if (type === "csv") {
        ...
        pdf.save(
          `${exportFilename || "export"}-${new Date().toISOString().slice(0, 10)}.pdf`,
        );
      }
      onClose();
    } catch (err) {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full h-10 px-3 rounded-lg border border-admin-200 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-neutral-500 mb-1">
