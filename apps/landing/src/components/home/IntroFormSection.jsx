...
function getStatIcon(label) {
  ...
  if (l.includes("trainer") || l.includes("expert") || l.includes("faculty"))
    return FiStar;
        ...
        return (
          <StaggerItem key={i} className="w-full">
            <div className="bg-white rounded-xl border border-gray-100 p-3.5 sm:p-4 text-center hover:-translate-y-1 transition-all duration-300 w-full h-full min-h-[88px] sm:min-h-[104px] flex flex-col items-center justify-center shadow-md">
              ...
              <p className="text-lg sm:text-2xl font-extrabold text-brand-blue leading-none">
                {stat.value}
              </p>
              <p className="text-[11px] sm:text-xs mt-1 text-slate-800 font-medium leading-snug">
                {stat.label}
              </p>
            </div>
          </StaggerItem>
      ...
      {pills.map((label, i) => (
        <StaggerItem key={i} className="min-w-0">
          <div className="flex items-center gap-1.5 sm:gap-2 bg-white rounded-lg border border-gray-100 px-2.5 sm:px-3 py-2.5 shadow-sm min-w-0 h-full min-h-[48px]">
            ...
            <span className="text-[11px] sm:text-xs font-bold text-brand-blue leading-tight whitespace-nowrap flex-1 min-w-0 truncate">
              {label}
            </span>
          </div>
        </StaggerItem>
...
export default function IntroFormSection({ section }) {
  ...
  const rawPills = Array.isArray(content.pill_buttons)
    ? content.pill_buttons
    : (content.pill_buttons || "").split("\n").filter(Boolean);
    ...
    <section className="relative overflow-hidden bg-white">
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-10 sm:py-16">
        <div className="grid lg:grid-cols-6 gap-6 sm:gap-8 lg:gap-12">
          <Reveal
            variant="up"
            className="lg:col-span-4 space-y-4 sm:space-y-6 self-start text-center lg:text-left"
          >
            {heading && (
          ...
          <Reveal
            variant="right"
            className="lg:col-span-2 self-start w-full max-w-md mx-auto lg:max-w-none"
          >
            <p
              className="text-sm sm:text-base font-[600] text-center mb-2"
              style={{ color: "#ef4444" }}
            >
              Book Your Demo Now!
            </p>
            <div
              className="rounded-2xl overflow-hidden max-w-sm w-full mx-auto lg:ml-auto"
              style={{
                backgroundColor: "#74a916",
            ...
            >
              ...
              >
                ...
                <div className="absolute inset-0 flex items-center justify-end">
                  <span
                    className="bg-white rounded-[6px] px-3 py-1 text-base font-serif font-bold shadow-sm mr-1.5"
                    style={{ color: "#f59e0b" }}
                  >
                    Counselling
                  </span>
              ...
              <div className="relative p-4 sm:p-5">
                <div
                  ...
                  style={{
                    backgroundImage:
                      "url(\"data:image/svg+xml,%3Csvg width='100' height='100' viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M10 50 Q 30 20 50 50 T 90 50' stroke='white' fill='none' stroke-width='2'/%3E%3C/svg%3E\")",
                ...
                >
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="w-full flex items-center justify-center gap-2 px-[30px] py-[15px] bg-[#f59e0b] text-white font-semibold rounded hover:bg-[#f59e0b]/90 transition-colors disabled:opacity-70 text-sm cursor-pointer active:scale-95"
                  >
                    {submitting ? (
