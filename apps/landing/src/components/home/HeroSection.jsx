...
export default function HeroSection({ section }) {
  ...
  const bannerImage = carouselEnabled
    ? carouselType === "image"
      ? slide?.image
      : ""
    : content.banner_image || "";
  const bannerHeading = carouselEnabled
    ? slide?.heading || ""
    : content.banner_heading || "";
  const bannerDescription = carouselEnabled
    ? slide?.description || ""
  ...
  const featureBullets = Array.isArray(rawBullets)
    ? rawBullets
    : rawBullets
      ? rawBullets.split("\n").filter(Boolean)
      : [];
  ...
  return (
    <section className="relative overflow-hidden">
      {bannerImage && (
        <div className="relative w-full overflow-hidden">
          <img
            src={bannerImage}
            alt=""
            className="w-full h-auto object-cover max-h-[350px] sm:max-h-none"
          />
          ...
          {(bannerHeading || bannerDescription) && (
            <div className="absolute inset-0 flex items-center">
              <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                {bannerHeading && (
                  <h1 className="text-[clamp(1.75rem,4vw,3.25rem)] font-extrabold text-white leading-[1.15] text-pretty">
                    {bannerHeading}
                  </h1>
                )}
                {bannerDescription && (
                  <p className="mt-3 sm:mt-4 text-base sm:text-lg text-white/85 leading-relaxed max-w-xl">
                    {bannerDescription}
                  </p>
                )}
              </div>
      ...
      {showGradient && (
        <div
          style={{
            background: "linear-gradient(135deg, #f59e0b 50%, #1B3A6B 50%)",
          }}
        >
              ...
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-24">
                <div className="grid lg:grid-cols-2 gap-10 sm:gap-12 lg:gap-16">
                  <motion.div
                    variants={container}
                    {...mount}
                    className="flex flex-col justify-center text-center items-center lg:text-left lg:items-start"
                  >
                    {badgeText && (
                      <Reveal
                        as="div"
                        className="inline-flex items-center gap-2 bg-white/15 backdrop-blur-sm rounded-full px-4 py-1.5 text-sm font-semibold text-white w-fit mb-6 mx-auto lg:mx-0"
                      >
                    ...
                    {description && (
                      <motion.p
                        variants={item}
                        className="mt-4 sm:mt-5 text-base sm:text-lg text-white/85 leading-relaxed max-w-xl mx-auto lg:mx-0"
                      >
                        {description}
                      </motion.p>
                    )}
                        ...
                        {buttons.map((btn, i) =>
                          btn.link ? (
                            <Link
                              ...
                              style={{
                                backgroundColor: btn.color || "#f59e0b",
                              }}
                            >
                              {btn.label}
                            </Link>
