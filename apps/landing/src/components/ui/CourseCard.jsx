...
export default function CourseCard({
  ...
  const titleSize =
    bannerSize === "lg" ? "text-lg sm:text-xl" : "text-base sm:text-lg";
      ...
      queryClient.prefetchQuery({
        ...
        queryFn: () =>
          supabase
            .from("courses")
            .select(
              "*, highlights(*), overview_faqs(*), course_fees(*), projects(*), certifications(*), course_tabs(*), faqs(*)",
            )
            .eq("slug", course.slug)
  ...
  function renderBanner() {
    ...
    if (variant === "initial") {
      return (
        <span className="text-white/15 text-4xl sm:text-5xl font-bold">
  ...
  return (
    ...
    >
      <div
        className={`${bannerH} bg-gradient-to-br from-brand-blue to-dark-navy flex items-center justify-center shrink-0 overflow-hidden`}
      >
      ...
      <div className={`${pad} flex flex-col flex-1`}>
        <h3
          className={`font-bold text-dark-navy ${titleSize} group-hover:text-brand-orange transition-colors`}
        >
          {course.title}
        </h3>
        <p className="text-sm text-text-gray mt-2 line-clamp-2 flex-1">
          {course.description}
        </p>
        ...
        <div
          className={`flex items-center ${showViewLink ? "justify-between" : "gap-4"} pt-4 border-t border-gray-100 text-sm text-text-gray mt-auto`}
        >
          <div className="flex items-center gap-3">
            ...
            <span>
              {(course.learner_count || 0).toLocaleString()}{" "}
              {bannerSize === "lg" ? "learners" : "Learners"}
            </span>
          </div>
          {showViewLink && (
