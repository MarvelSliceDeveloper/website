...
```json
{
  ...
  "published": true,
  "checklist": [{ "item": "Course has a title", "passed": true }]
}
```
...
```json
{
  ...
  "checklist": [{ "item": "Course has a title", "passed": false }]
}
```
  ...
  "recordings": [
    {
      ...
      "progress": [{ "watchedSeconds": 1200, "completedAt": null }]
    }
  ]
}
```
...
await api.post("/api/mentorship/tickets", {
  ...
  description:
    "I want help choosing and interpreting metrics for my classification project.",
...
| Method | Path                                | Auth | Description                                            |
| ------ | ----------------------------------- | ---- | ------------------------------------------------------ |
| None   | _(router has no registered routes)_ | —    | Controller & service exist but endpoints are not wired |
...
| `accessToken` | The JWT token received from login.                         | _Leave blank initially_ |
| `courseId`    | A Course ID for creating sessions or getting recordings.   | _Leave blank initially_ |
| `moduleId`    | A Module ID for scheduling sessions.                       | _Leave blank initially_ |
| `sessionId`   | A Session ID returned from the Create Session endpoint.    | _Leave blank initially_ |
| `recordingId` | A Recording ID returned from the List Recordings endpoint. | _Leave blank initially_ |
...
- _Note:_ Adjust dates to fall within your testing timeframe.
...
_(Requires `courseId` and `moduleId` to be set in your environment if you already have them in your DB)_
...
_(These endpoints return data only if your session has synced recordings)_
